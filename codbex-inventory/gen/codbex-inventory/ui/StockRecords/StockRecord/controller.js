angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, $http, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('codbex-inventory:defaults.deleteTitle', { name: '$t(codbex-inventory:t.STOCKRECORD)' });
			translated.deleteConfirm = LocaleService.t('codbex-inventory:messages.deleteConfirm', { name: '$t(codbex-inventory:t.STOCKRECORD)' });
		});
		$scope.dataPage = 1;
		$scope.dataCount = 0;
		$scope.dataOffset = 0;
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('codbex-inventory:t.STOCKRECORD'),
							message: LocaleService.t('codbex-inventory:messages.error.unableToDelete', { name: '$t(codbex-inventory:t.STOCKRECORD)', message: message }),
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
