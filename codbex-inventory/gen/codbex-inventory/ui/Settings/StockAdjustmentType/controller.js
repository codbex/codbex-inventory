angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('codbex-inventory:defaults.deleteTitle', { name: '$t(codbex-inventory:t.STOCKADJUSTMENTTYPE)' });
			translated.deleteConfirm = LocaleService.t('codbex-inventory:messages.deleteConfirm', { name: '$t(codbex-inventory:t.STOCKADJUSTMENTTYPE)' });
		});
			...
			EntityService.count(filter).then((resp) => {
				...
				request.then((response) => {
					...
					Dialogs.showAlert({
						...
						message: LocaleService.t('codbex-inventory:messages.error.unableToLF', { name: '$t(codbex-inventory:t.STOCKADJUSTMENTTYPE)', message: message }),
				...
				Dialogs.showAlert({
					title: LocaleService.t('codbex-inventory:t.STOCKADJUSTMENTTYPE'),
					message: LocaleService.t('codbex-inventory:messages.error.unableToCount', { name: '$t(codbex-inventory:t.STOCKADJUSTMENTTYPE)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then((response) => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('codbex-inventory:t.STOCKADJUSTMENTTYPE'),
							message: LocaleService.t('codbex-inventory:messages.error.unableToDelete', { name: '$t(codbex-inventory:t.STOCKADJUSTMENTTYPE)', message: message }),
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
