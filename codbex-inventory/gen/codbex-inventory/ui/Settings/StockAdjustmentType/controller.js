angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		let translated = {
			yes: 'Yes',
			no: 'No',
			deleteConfirm: 'Are you sure you want to delete StockAdjustmentType? This action cannot be undone.',
			deleteTitle: 'Delete StockAdjustmentType?'
		};
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('codbex-inventory:codbex-inventory-model.defaults.deleteTitle', { name: '$t(codbex-inventory:codbex-inventory-model.t.STOCKADJUSTMENTTYPE)' });
			translated.deleteConfirm = LocaleService.t('codbex-inventory:codbex-inventory-model.messages.deleteConfirm', { name: '$t(codbex-inventory:codbex-inventory-model.t.STOCKADJUSTMENTTYPE)' });
		});
			...
			EntityService.count(filter).then((resp) => {
				...
				request.then((response) => {
					...
					Dialogs.showAlert({
						...
						message: LocaleService.t('codbex-inventory:codbex-inventory-model.messages.error.unableToLF', { name: '$t(codbex-inventory:codbex-inventory-model.t.STOCKADJUSTMENTTYPE)', message: message }),
				...
				Dialogs.showAlert({
					title: LocaleService.t('codbex-inventory:codbex-inventory-model.t.STOCKADJUSTMENTTYPE'),
					message: LocaleService.t('codbex-inventory:codbex-inventory-model.messages.error.unableToCount', { name: '$t(codbex-inventory:codbex-inventory-model.t.STOCKADJUSTMENTTYPE)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then((response) => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('codbex-inventory:codbex-inventory-model.t.STOCKADJUSTMENTTYPE'),
							message: LocaleService.t('codbex-inventory:codbex-inventory-model.messages.error.unableToDelete', { name: '$t(codbex-inventory:codbex-inventory-model.t.STOCKADJUSTMENTTYPE)', message: message }),
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
