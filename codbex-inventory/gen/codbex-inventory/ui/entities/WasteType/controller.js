angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', 'messageHub', 'entityApi', 'Extensions', function ($scope, messageHub, entityApi, Extensions) {
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("WasteType", `Unable to count WasteType: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				request.then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("WasteType", `Unable to list/filter WasteType: '${response.message}'`);
						return;
					}
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("WasteType-filter", {
		...
		$scope.deleteEntity = function (entity) {
			...
			messageHub.showDialogAsync(
				'Delete WasteType?',
				`Are you sure you want to delete WasteType? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("WasteType", `Unable to delete WasteType: '${response.message}'`);
							return;
						}
						$scope.loadPage($scope.dataPage, $scope.filter);
