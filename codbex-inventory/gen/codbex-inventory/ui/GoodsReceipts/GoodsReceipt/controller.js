angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, $http, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('codbex-inventory:codbex-inventory-model.defaults.deleteTitle', { name: '$t(codbex-inventory:codbex-inventory-model.t.GOODSRECEIPT)' });
			translated.deleteConfirm = LocaleService.t('codbex-inventory:codbex-inventory-model.messages.deleteConfirm', { name: '$t(codbex-inventory:codbex-inventory-model.t.GOODSRECEIPT)' });
		});
		$scope.dataPage = 1;
		$scope.dataCount = 0;
		$scope.dataOffset = 0;
			...
			EntityService.count(filter).then((resp) => {
				...
				EntityService.search(filter).then((response) => {
					...
					Dialogs.showAlert({
						...
						message: LocaleService.t('codbex-inventory:codbex-inventory-model.messages.error.unableToLF', { name: '$t(codbex-inventory:codbex-inventory-model.t.GOODSRECEIPT)', message: message }),
				...
				Dialogs.showAlert({
					title: LocaleService.t('codbex-inventory:codbex-inventory-model.t.GOODSRECEIPT'),
					message: LocaleService.t('codbex-inventory:codbex-inventory-model.messages.error.unableToCount', { name: '$t(codbex-inventory:codbex-inventory-model.t.GOODSRECEIPT)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('codbex-inventory:codbex-inventory-model.t.GOODSRECEIPT'),
							message: LocaleService.t('codbex-inventory:codbex-inventory-model.messages.error.unableToDelete', { name: '$t(codbex-inventory:codbex-inventory-model.t.GOODSRECEIPT)', message: message }),
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
