angular.module('page', ['blimpKit', 'platformView', 'platformLocale', 'EntityService'])
	...
	.controller('PageController', ($scope, $http, EntityService, Extensions, LocaleService, ButtonStates) => {
		...
		let translated = {
			yes: 'Yes',
			no: 'No',
			deleteConfirm: 'Are you sure you want to delete DeliveryNote? This action cannot be undone.',
			deleteTitle: 'Delete DeliveryNote?'
		};
		...
		LocaleService.onInit(() => {
			...
			translated.deleteTitle = LocaleService.t('codbex-inventory:codbex-inventory-model.defaults.deleteTitle', { name: '$t(codbex-inventory:codbex-inventory-model.t.DELIVERYNOTE)' });
			translated.deleteConfirm = LocaleService.t('codbex-inventory:codbex-inventory-model.messages.deleteConfirm', { name: '$t(codbex-inventory:codbex-inventory-model.t.DELIVERYNOTE)' });
		});
		$scope.dataPage = 1;
		$scope.dataCount = 0;
		$scope.dataOffset = 0;
			...
			EntityService.count(filter).then((resp) => {
				...
				EntityService.search(filter).then((response) => {
					...
					Dialogs.showAlert({
						...
						message: LocaleService.t('codbex-inventory:codbex-inventory-model.messages.error.unableToLF', { name: '$t(codbex-inventory:codbex-inventory-model.t.DELIVERYNOTE)', message: message }),
				...
				Dialogs.showAlert({
					title: LocaleService.t('codbex-inventory:codbex-inventory-model.t.DELIVERYNOTE'),
					message: LocaleService.t('codbex-inventory:codbex-inventory-model.messages.error.unableToCount', { name: '$t(codbex-inventory:codbex-inventory-model.t.DELIVERYNOTE)', message: message }),
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
			...
			Dialogs.showDialog({
				...
				if (buttonId === 'delete-btn-yes') {
					EntityService.delete(id).then(() => {
						...
						Dialogs.showAlert({
							title: LocaleService.t('codbex-inventory:codbex-inventory-model.t.DELIVERYNOTE'),
							message: LocaleService.t('codbex-inventory:codbex-inventory-model.messages.error.unableToDelete', { name: '$t(codbex-inventory:codbex-inventory-model.t.DELIVERYNOTE)', message: message }),
							type: AlertTypes.Error
						});
						console.error('EntityService:', error);
					});
