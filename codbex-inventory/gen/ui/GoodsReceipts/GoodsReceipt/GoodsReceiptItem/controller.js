angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-inventory-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "GoodsReceipts" && e.view === "GoodsReceiptItem" && (e.type === "page" || e.type === undefined));
			$scope.entityActions = response.filter(e => e.perspective === "GoodsReceipts" && e.view === "GoodsReceiptItem" && e.type === "entity");
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("GoodsReceiptItem", `Unable to list/filter GoodsReceiptItem: '${response.message}'`);
						return;
					}
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("GoodsReceiptItem-filter", {
