angular.module('page', ['blimpKit', 'platformView', 'EntityService'])
	...
	.controller('PageController', ($scope, EntityService, Extensions) => {
		...
		Extensions.getWindows(['codbex-inventory-custom-action']).then((response) => {
			$scope.pageActions = response.data.filter(e => e.perspective === 'Reports' && e.view === 'InventoryAvailability' && (e.type === 'page' || e.type === undefined));
			$scope.entityActions = response.data.filter(e => e.perspective === 'Reports' && e.view === 'InventoryAvailability' && e.type === 'entity');
		});
		...
		//-----------------Events-------------------//
		Dialogs.addMessageListener({ topic: 'codbex-inventory.Reports.InventoryAvailability.entitySearch', handler: (data) => {
			resetPagination();
		...
		$scope.loadPage = (pageNumber, filter) => {
			...
			EntityService.count(filter).then((resp) => {
				...
				Dialogs.showAlert({
					title: 'InventoryAvailability',
					message: `Unable to count InventoryAvailability: '${message}'`,
					type: AlertTypes.Error
				});
				console.error('EntityService:', error);
			});
